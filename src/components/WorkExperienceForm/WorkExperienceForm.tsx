...
function WorkExperienceForm({jobs, updateWorkExperience}: WorkExperienceFormProps) {
  ...
  const handleChange = (id: string, key: keyof Job) => (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
  ...
  return (
    <form>
      ...
      {!isHidden && <div>
        ...
        <div className="jobs">
        {jobs.map((job) => (
          <div key={job.id} onClick={showWorkFields}>{job.company}</div>
        ))}
      </div>
      )}
      {isHidden && (
